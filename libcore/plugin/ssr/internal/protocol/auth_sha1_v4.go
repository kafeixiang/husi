...
func (a *authSHA1V4) packData(poolBuf *bytes.Buffer, data []byte) {
	...
	var tmp [4]byte
	binary.BigEndian.PutUint16(tmp[:2], uint16(packedDataLength))
	poolBuf.Write(tmp[:2])

	binary.LittleEndian.PutUint16(tmp[:2], uint16(crc32.ChecksumIEEE(poolBuf.Bytes()[poolBuf.Len()-2:])&0xffff))
	poolBuf.Write(tmp[:2])

	a.packRandData(poolBuf, randDataLength)
	poolBuf.Write(data)

	binary.LittleEndian.PutUint32(tmp[:4], adler32.Checksum(poolBuf.Bytes()[poolBuf.Len()-packedDataLength+4:]))
	poolBuf.Write(tmp[:4])
}
...
func (a *authSHA1V4) packAuthData(poolBuf *bytes.Buffer, data []byte) {
	...
	binary.LittleEndian.PutUint32(tmp[:4], crc32.ChecksumIEEE(crcData))
	poolBuf.Write(tmp[:4])
...
func (a *authSHA1V4) packRandData(poolBuf *bytes.Buffer, size int) {
	...
	var tmp [2]byte
	binary.BigEndian.PutUint16(tmp[:], uint16(size+3))
	poolBuf.Write(tmp[:])
	tools.AppendRandBytes(poolBuf, size)
}
