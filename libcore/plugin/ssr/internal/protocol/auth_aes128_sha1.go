...
func (a *authAES128) initUserData() {
	...
	if len(params) > 1 {
		if userID, err := strconv.ParseUint(params[0], 10, 32); err == nil {
			...
			log.Printf("Wrong protocol-param for %s, only digits are expected before ':'", a.salt)
		}
	}
	if len(a.userKey) == 0 {
		a.userKey = a.Key
		_, _ = crand.Read(a.userID[:])
	}
}
...
func (a *authAES128) packData(poolBuf *bytes.Buffer, data []byte, fullDataLength int) {
	...
	var tmp [2]byte
	binary.LittleEndian.PutUint16(tmp[:], uint16(packedDataLength))
	poolBuf.Write(tmp[:])
...
func (a *authAES128) packRandData(poolBuf *bytes.Buffer, size int) {
	...
	var tmp [2]byte
	binary.LittleEndian.PutUint16(tmp[:], uint16(size+3))
	poolBuf.Write(tmp[:])
	tools.AppendRandBytes(poolBuf, size)
}
