...
const (
	...
	DefaultRateLimitRetryDelay = 1 * time.Minute // 53-55 request / min
...
func isHTTPErrorCode(err error, code int) bool {
	if apiError, isAPIError := errors.AsType[*pkgsite.APIError](err); isAPIError {
		return apiError.Code == code
	}
	if httpError, isHTTPError := errors.AsType[pkgsite.HTTPError](err); isHTTPError {
