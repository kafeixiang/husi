...
fun buildConfig(
    ...
    val tagDnsFinal = if (DNSMode.isLeakMode(DataStore.dnsMode)) {
        TAG_DNS_DIRECT
    } else {
        TAG_DNS_REMOTE
    }
...
//                        val needKeepAliveInterval = keepAliveInterval !in intArrayOf(0, 15)
        ...
        directDNS.firstOrNull().let {
            dns.servers.add(DNSServerOptions().apply {
                ...
                if (DataStore.directDnsClientSubnet.isNotBlank()) {
                    client_subnet = DataStore.directDnsClientSubnet
                } else if (DNSMode.isLeakMode(DataStore.dnsMode)) {
                    throw Exception("Your DNS mode requires set direct DNS client subnet.")
                }
            })
        }
